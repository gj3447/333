...
fn a_valid_spend_finalizes_and_advances_the_version() {
    ...
    // registration is itself a shipped transition (wal design §1 gap 1)
    assert_eq!(verify_present(&s, "tx-1", "object_registered", 1), Verdict::Present);
}
...
fn a_stale_spend_after_finalize_is_also_rejected() {
    let mut s = MemoryStore::default();
    let mut l = Ledger::default();
    l.register(&mut s, "tx-stale", "coin-A");
